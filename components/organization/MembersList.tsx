...
export function MembersList({ members }: { members: Member[] }) {
  ...
  const activeStatuses = [...new Set(members.map(m => m.status))]
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-1 flex-wrap">
          <FilterPill active={statusFilter === 'all'} onClick={() => setStatusFilter('all')} label="All" count={members.length} />
      ...
      <div className="border rounded-lg divide-y overflow-hidden bg-card">
        {filtered.map((member) => {
          const status = statusConfig[member.status] || statusConfig.new_member
          ...
          return (
            ...
            >
              ...
              {/* Linked badge */}
              {member.volunteer_id && (
                <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 text-[10px] font-medium flex-shrink-0">
