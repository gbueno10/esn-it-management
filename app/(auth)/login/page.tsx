...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      // If volunteer signup, call the volunteer setup API
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
          <div className="p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {mode === 'signup' && (
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-500 ml-1 uppercase tracking-wider">Confirm Password</label>
                  <div className="relative group">
                    <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-[var(--primary)] transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {error && (
                <div className="p-4 rounded-xl text-sm bg-red-50 text-red-600 border border-red-100">{error}</div>
              )}
